...
from config.main_config3 import *
...
def main(test_instance, test_instance_date, run, repair_removed, destroy_removed, naive, adaptive, standby):
    ...
    try:
        ...
        df_run.append([run, "Initial", total_objective.total_seconds(), (datetime.now() - start_time).total_seconds(), cumulative_rejected, rejected_objective.total_seconds(),
                       cumulative_recalibration.total_seconds()/beta_reopt, ride_time_objective.total_seconds(), ride_sharing, ride_sharing_arcs, ride_sharing_passengers, passengers_total, len(current_route_plan), "10:00"])
        print("Initial objective", current_objective.total_seconds())
        print("Initial rejected", cumulative_rejected)
        cumulative_deviation = copy(cumulative_recalibration)/beta_reopt
        ...
        new_request_updater = NewRequestUpdater(
            constructor, standby, alpha_reopt, beta_reopt)
        ...
        while len(simulator.disruptions_stack) > 0:
            ...
            if not naive and not rejection:
                ...
                if delayed[0]:
                    ...
                    cumulative_recalibration += delta_dev_objective
                    current_objective -= delta_dev_objective
                    cumulative_deviation += delta_dev_objective/beta_reopt
...
if __name__ == "__main__":
    ...
    if run == 1:
        ...
        df_track_req_runtime.to_csv(
            config("run_path") + "Stack" + + test_instance + "runtime_reqs" + ".csv")

        df_cancel_total = pd.concat(df_cancel_runs)
        df_cancel_total.to_csv(
            config("run_path") + "Stack" + test_instance + "cancel_info" + ".csv")

    df_track_run = pd.concat(df_runs)
    df_track_run.to_csv(
        config("run_path") + "Tuning_with_weights:" + str(alpha_reopt) + "&" + str(beta_reopt) + "_Run:" + str(run) + test_instance + "analysis" + ".csv")
