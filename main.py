...
if __name__ == "__main__":
    ...
    if run == 1:
        ...
        df_track_req_runtime.to_csv(
            config("run_path") + "Semi_Naive" + str(naive) + test_instance + "runtime_reqs" + ".csv")

        df_cancel_total = pd.concat(df_cancel_runs)
        df_cancel_total.to_csv(
            config("run_path") + "Semi_Naive" + str(naive) + test_instance + "cancel_info" + ".csv")

    df_track_run = pd.concat(df_runs)
    df_track_run.to_csv(
        config("run_path") + "Semi_Naive" + str(naive) + "_Run:" + str(run) + test_instance + "analysis" + ".csv")
